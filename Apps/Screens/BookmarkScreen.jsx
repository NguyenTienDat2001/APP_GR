...
export default function BookmarkScreen() {
  ...
  const getBooks = async () => {
    try {
      ...
      const response = await axios.get(`${Domain.domainUrl}/books/bookmark/all`, {
        headers: {
          'Authorization': token
        },
      });
  ...
  const cancelBookmark = async (book_id) => {
    console.log(book_id);
    const token = await AsyncStorage.getItem('token');
    ...
    <View className=' px-1 bg-white mb-2 p-2'>
      <TouchableOpacity>
        <View className='flex flex-row items-center gap-3 px-1'>
          ...
          <TouchableOpacity onPress={() => cancelBookmark(item.book_id)}>
            <View><Text className='text-red-400'>Xóa</Text></View>
          </TouchableOpacity>
        </View>
      </TouchableOpacity>
    </View>
